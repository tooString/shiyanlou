...
from falsk import request, make_response
from flask.json import dumps
from flask.views import MethodView
from werzeug.wrappers import Response
...
class RestView(MethodView):
    ...
    def dispatch_request(self, *args, **kwargs):
        ...
        assert method is not None, 'Unimplemented method %r' % request.method_decorators
        ...
        # 如果返回结果已经是HTTP相应则直接返回
        if isinstance(resp, Response):
            return resp

        # 从返回值中解析出HTTP相应信息，比如状态码和头部
        data, code, headers = RestView.unpack(resp)

        # 处理错误，HTTP状态码大于 400 时认为是错误
        # 返回的错误类似于 {'name': ['redis server alreadly exist']} 将其调整为
        # {'ok': False, 'message': 'redis server alreadly exist'}
